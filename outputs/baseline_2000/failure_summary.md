...
Total failed text-to-image queries analyzed: 4985

## Category Counts

- `generic_human_activity`: 2895 failures (58.1%). Short person-centric captions with weak visual specificity, such as a person walking, standing, or waiting outside.
- `multiple_objects`: 1941 failures (38.9%). Captions involving several entities or relationships, which makes one-to-one image matching harder.
- `fine_grained_attributes`: 1079 failures (21.6%). Captions that depend on subtle attributes such as clothing, colors, pose, or small object details.
- `crowded_scene`: 856 failures (17.2%). Captions describing crowds, groups, or busy public scenes where many similar people/objects compete for attention.
- `unusual_scene`: 42 failures (0.8%). Captions describing uncommon, stylized, or visually atypical scenes.
- `ambiguous_query`: 26 failures (0.5%). Captions containing words with multiple meanings or underspecified object references.

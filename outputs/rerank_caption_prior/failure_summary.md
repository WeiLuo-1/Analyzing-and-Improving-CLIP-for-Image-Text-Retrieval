...
Total failed text-to-image queries analyzed: 209

## Category Counts

- `generic_human_activity`: 126 failures (60.3%). Short person-centric captions with weak visual specificity, such as a person walking, standing, or waiting outside.
- `multiple_objects`: 93 failures (44.5%). Captions involving several entities or relationships, which makes one-to-one image matching harder.
- `fine_grained_attributes`: 50 failures (23.9%). Captions that depend on subtle attributes such as clothing, colors, pose, or small object details.
- `crowded_scene`: 37 failures (17.7%). Captions describing crowds, groups, or busy public scenes where many similar people/objects compete for attention.
- `unusual_scene`: 1 failures (0.5%). Captions describing uncommon, stylized, or visually atypical scenes.
...
- Rank 52: "A man in bright pants is pushing a cart ." (gt=27, pred=71)
